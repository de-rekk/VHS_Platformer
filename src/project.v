...
module tt_um_vga_platformer(
  ...
  // --------------------------------------------------------
  // OPTIMIZED PHYSICS DATAPATH (Calculated Once)
  // --------------------------------------------------------
  wire [10:0] nx_r = player_x + 11'd3;
  wire [10:0] nx_l = player_x - 11'd3;
  
  wire wall_r = (get_floor((nx_r + 11'd15) >> 5) < (player_y + 10'd14));
  wire wall_l = (get_floor(nx_l >> 5) < (player_y + 10'd14));
  
  wire [10:0] nx = (btn_right && !wall_r && player_x < 11'd2000) ? nx_r :
                   (btn_left  && !wall_l && player_x > 11'd16)   ? nx_l : 
                   player_x;
                   
  wire [5:0] col_nx_l = nx[10:5];
  wire [5:0] col_nx_r = (nx + 11'd15) >> 5;
  
  wire [9:0] fl_a = get_floor(col_nx_l);
  wire [9:0] fl_b = get_floor(col_nx_r);
  wire [9:0] fl_actual = (fl_a < fl_b) ? fl_a : fl_b;
  
  wire signed [5:0] vt = (grounded && btn_jump) ? -6'sd14 : player_vy;
  wire [9:0] ny_cand = player_y + {{4{vt[5]}}, vt};
  
  wire landing = (!vt[5]) && ((ny_cand + 10'd16) >= fl_actual);
  
  wire [9:0] ny = landing ? (fl_actual - 10'd16) : ny_cand;
  wire signed [5:0] vn = landing ? 6'sd0 : ((vt < 6'sd12) ? (vt + 6'sd1) : vt);
  wire grn = landing;
  
  wire hit_spike = (get_spike(col_nx_l) || get_spike(col_nx_r)) && 
                   ((ny + 10'd16) > (fl_actual - 10'd10));

  wire [10:0] next_cam_x = (nx < 11'd320) ? 11'd0 : 
                           (nx > 11'd1728) ? 11'd1408 : 
                           (nx - 11'd320);
  ...
  wire [10:0] world_x = {1'b0, hpos} + cam_x;
  ...
  wire is_player = (game_state != 2'd2) && (hpos >= px_screen) && (hpos < px_screen + 10'd16) && 
                   (vpos >= player_y) && (vpos < player_y + 10'd16);

  wire is_ground = (vpos >= render_floor_y);
  wire is_edge   = is_ground && (vpos < (render_floor_y + 10'd3));
  wire is_spike  = render_has_spike && (vpos >= (render_floor_y - 10'd16)) && !is_ground && (world_x[3] ^ vpos[3]);
  wire is_door   = ((col_index == 6'd60) || (col_index == 6'd61)) && (vpos >= (render_floor_y - 10'd48)) && !is_ground;

  // Simple rectangle to save hundreds of arithmetic gates
  wire is_play_icon = (hpos >= 10'd40) && (hpos <= 10'd56) && (vpos >= 10'd30) && (vpos <= 10'd46) && 
                      (game_state == 2'd0 || game_state == 2'd2 || game_state == 2'd3);
